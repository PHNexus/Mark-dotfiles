...
cava() {
    spawn kitty --title "markcava" --class "seycava" -o font_size=$FONT_GENERAL sh -c "cava"
    sleep 0.2
}

lavat() {
    spawn kitty --title "marklavat" --class "seylavat" -o font_size=$FONT_GENERAL sh -c "lavat -c blue -k blue -r1"
    sleep 0.2
}

clock() {
    spawn kitty --title "markclock" --class "seyclock" -o font_size=$FONT_CLOCK sh -c "tty-clock -c -C 4 -r -b"
    sleep 0.2
}

cmd() {
    spawn kitty --title "markcmd" --class "seycmd" -o font_size=$FONT_TERMINAL --hold fastfetch
    sleep 0.2
}
